function RIR = genMultiChannelRIR(t60, useGPU,doa)
if nargin<2
    useGPU = 0;
end
...
dist=0.2;
room_size = [7 5 3];   % length, width, and height of the room
ctr_pos = room_size./2    % center position of the array
src_pos = repmat(ctr_pos,1,1) + [dist.*sin(doa.*pi./180)',dist.*cos(doa.*pi./180)',zeros(1,1)];
% src_pos = [3 3 2];    % coordinate of the sound source
[mic_pos] = genMicPositionsWrtCenter_coprime(0);        % get the microphones' positions with respect to the center of the array
